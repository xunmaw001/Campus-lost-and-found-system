<template>
    <div class="main-content">
        <el-dialog title="审核" :visible.sync="xunwuYuyueYesnoTypesVisible">
            <el-form :model="form">
                <input type="hidden" v-model="form.id">
                <el-form-item label="审核" >
                    <el-select v-model="form.xunwuYuyueYesnoTypes" placeholder="请选择报名状态">
                        <el-option label="同意" value="2"></el-option>
                        <el-option label="拒绝" value="3"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="审核回复">
                    <el-input type="textarea" v-model="form.xunwuYuyueYesnoText" placeholder="审核回复"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="xunwuYuyueYesnoTypesVisible = false">取 消</el-button>
                <el-button type="primary" @click="xunwuYuyueYesnoTypesShenhe">审 核</el-button>
            </div>
        </el-dialog>
        <!-- 条件查询 -->
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '报名编号' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.xunwuYuyueUuidNumber" placeholder="报名编号" clearable></el-input>
                     </el-form-item>
                                                 
                     <el-form-item :label="contents.inputTitle == 1 ? '报名状态' : ''">
                        <el-select v-model="searchForm.xunwuYuyueYesnoTypes" placeholder="请选择报名状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in xunwuYuyueYesnoTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('xunwuYuyue','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiaoyaunshiwuzhaolingxitong/upload/xunwuYuyueMuBan.xls"
                        >批量导入寻物认领数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('xunwuYuyue','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="xunwuYuyueUploadSuccess"
                                :on-error="xunwuYuyueUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('xunwuYuyue','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入寻物认领数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('xunwuYuyue','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "xunwuYuyue.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="xunwuYuyueUuidNumber"
                                   header-align="center"
                                   label="报名编号">
                        <template slot-scope="scope">
                            {{scope.row.xunwuYuyueUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xunwuYuyueText"
                                      ...
                                      label="报名理由">
                        <template slot-scope="scope">
                            <span v-if="scope.row.xunwuYuyueText != null &&scope.row.xunwuYuyueText.length>10">
                                {{scope.row.xunwuYuyueText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.xunwuYuyueText}}
                            </span>
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="xunwuYuyuePhoto"
                                     header-align="center"
                                     width="200"
                                     label="照片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.xunwuYuyuePhoto">
                                <img :src="$base.url+scope.row.xunwuYuyuePhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xunwuYuyueYesnoTypes"
                                      header-align="center"
                                      label="报名状态">
                        <template slot-scope="scope">
                            {{scope.row.xunwuYuyueYesnoValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xunwuYuyueYesnoText"
                                      header-align="center"
                                      label="审核回复">
                        <template slot-scope="scope">
                            <span v-if="scope.row.xunwuYuyueYesnoText != null &&scope.row.xunwuYuyueYesnoText.length>10">
                                {{scope.row.xunwuYuyueYesnoText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.xunwuYuyueYesnoText}}
                            </span>
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="xunwuYuyueShenheTime"
                                   header-align="center"
                                   label="审核时间">
                        <template slot-scope="scope">
                            {{scope.row.xunwuYuyueShenheTime}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('xunwuYuyue','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('xunwuYuyue','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('xunwuYuyue','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('xunwuYuyue','审核') && scope.row.xunwuYuyueYesnoTypes == 1 " type="primary" icon="el-icon-thumb" size="mini" @click="openYesnoTypes(scope.row.id)">审核</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              xunwuYuyueYesnoTypesSelectSearch : [],
            form:{
                id : null,
                xunwuYuyueUuidNumber : null,
                xunwuId : null,
                yonghuId : null,
                xunwuYuyueText : null,
                xunwuYuyuePhoto : null,
                insertTime : null,
                xunwuYuyueYesnoTypes : null,
                xunwuYuyueYesnoText : null,
                xunwuYuyueShenheTime : null,
                createTime : null,
            },
            dataList: [],
            pageIndex: 1,
            ...
            xunwuYuyueYesnoTypesVisible : false,
            ...
            json_fields: {
                //级联表字段
                     ...
                     '用户邮箱': 'yonghuEmail',
                //本表字段
                     '报名编号': "xunwuYuyueUuidNumber",
                     '报名理由': "xunwuYuyueText",
                     '照片': "xunwuYuyuePhoto",
                     '寻物启示报名时间': "insertTime",
                     '报名状态': "xunwuYuyueYesnoValue",
                     '审核回复': "xunwuYuyueYesnoText",
                     '审核时间': "xunwuYuyueShenheTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'xunwu_yuyue',//当前表表名,
                        sumColum : 'xunwu_yuyue_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'xunwu_yuyue_name',//分组字符串字段
                        // types : 'xunwu_yuyue_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.xunwuYuyueUuidNumber!= '' && this.searchForm.xunwuYuyueUuidNumber!= undefined) {
                    params['xunwuYuyueUuidNumber'] = '%' + this.searchForm.xunwuYuyueUuidNumber + '%'
                }
                                                 
                if (this.searchForm.xunwuYuyueYesnoTypes!= '' && this.searchForm.xunwuYuyueYesnoTypes!= undefined) {
                    params['xunwuYuyueYesnoTypes'] = this.searchForm.xunwuYuyueYesnoTypes
                }
                        
                params['xunwuYuyueDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=xunwu_yuyue_yesno_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.xunwuYuyueYesnoTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            xunwuYuyueUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "xunwuYuyue/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            xunwuYuyueUploadError(data){
                ...
                _this.form.id = null;
                _this.form.id = id;
                            _this.form.xunwuYuyueYesnoText = null;
                _this.form.xunwuYuyueYesnoTypes = "请选择报名状态";
                _this.xunwuYuyueYesnoTypesVisible = true;
            },

            xunwuYuyueYesnoTypesShenhe() {
                let _this = this;
                if(_this.form.xunwuYuyueYesnoTypes == "请选择报名状态"){
                    _this.$message.error("请选择报名状态");return false;
                }
                if(_this.form.xunwuYuyueYesnoText == null || _this.form.xunwuYuyueYesnoText == ""){
                    _this.$message.error("请输入审核回复");return false;
                }
                this.$http({
                    url:`xunwuYuyue/shenhe`,
                    method: "post",
                    data: _this.form
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.$message({
                            ...
                            duration: 1500,
                            onClose: () => {
                                _this.xunwuYuyueYesnoTypesVisible = false;
                                _this.search();
                            }
                        });
                    } else {
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "xunwuYuyue/update",
                        method: "post",
                        data: {
                            id:id,
//                            xunwuYuyueTypes:xunwuYuyueTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
